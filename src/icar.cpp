...
#include "recognition/ring_recognition.cpp"     /* 环岛识别类 */
...
int main(int argc, char const *argv[])
{
    ...
    RingRecognition ringRecognition;                /* 环岛检测类 */
    ...
    while (1)
    {
        ...
        else if (motionController.params.saveImage) //保存原始图像
        ...
        if (motionController.params.GarageEnable) //赛道元素是否使能
        {
            ...
            {
                countercircles++; //圈数计数
                if (countercircles > 500)   /* 200 */
                    countercircles = 500;
                    
                if (garageRecognition.startingCheck(resultAI->predictor_results)) //检测到起点（斑马线）
                {
                    busyareaDetection.reset();   //施工区检测初始化
                    freezoneRecognition.reset(); //泛行区识别复位

                    if (countercircles > 300)       //大于60认为是有效走了一圈？
                    {
                        circlesThis++;
                        countercircles = 0;
                    }
                    ...
                    if (motionController.params.saveImage)  //保存原始图像
                    {
                        ...
                        putText(imageGarage, "crosswork", Point(COLSIMAGE / 2 - 5, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 0, 255), 1, CV_AA);
            ...
            if (roadType == RoadType::RingHandle || roadType == RoadType::BaseHandle)
            {
                if (ringRecognition.ringRecognition(trackRecognition))
                {
                    ...
                    if (motionController.params.debug)
                    {
                        ...
                        ringRecognition.drawImage(trackRecognition, imageCross);
        ...
        controlCenterCal.controlCenterCal(trackRecognition, motionController.params.AreaRevise); /* 新增AreaRevise */
        ...
        if (motionController.params.debug)
        {
            ...
            /* 显示角度 */
            putText(imgaeCorrect, "a: " + formatDoble2String(motionController.servoPwm, 2), Point(COLSIMAGE - 90, 100), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 1);
