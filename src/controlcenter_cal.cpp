...
public:
    ...
    void controlCenterCal(TrackRecognition &track, float AreaRevise)    /* 新增AreaRevise */
    {
        sigmaCenter = 0;
        ...
        /*
        if ((track.stdevLeft < 30 && track.stdevRight > 100) || (track.stdevLeft > 100 && track.stdevRight < 30))
        {
            validRowsCal(track.pointsEdgeLeft, track.pointsEdgeRight); //杈圭紭鏈夋晥琛岃绠?
        ...
        */
        ...
        //     track.pointsEdgeRight.resize(validRowsRight);
        // }
        int controlCenterRevise = 0;
        if (AreaRevise != 0) {
            controlCenterRevise = computeCCRevise(track.pointsEdgeLeft, track.pointsEdgeRight, AreaRevise);
        }
        ...
        /* revise版 */
        controlCenter += controlCenterRevise;
        ...
        /* zht: 减小拐弯幅度以延迟
        controlCenter = (controlCenter + COLSIMAGE / 2) / 2; */
    ...
    void drawImage(TrackRecognition track, Mat &centerImage)
    {
        ...
        int dis = 20;
        string str;
        putText(centerImage, style, Point(COLSIMAGE - 80, dis), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 1); //赛道类型

        str = "Edge: " + formatDoble2String(track.stdevLeft, 1) + " | " + formatDoble2String(track.stdevRight, 1);
        putText(centerImage, str, Point(COLSIMAGE - 180, 2 * dis), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 1); //斜率：左|右

        str = "Center: " + formatDoble2String(sigmaCenter, 2);
        putText(centerImage, str, Point(COLSIMAGE - 140, 3 * dis), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 1); //中心点方差
    }
...
private:
    ...
    void  validRowsCal(vector<POINT>& pointsEdgeLeft, vector<POINT>& pointsEdgeRight) {
        //修正左侧
        float kBefore, kAfter;

        validRowsLeft = -1;
        for (int i = 5; i < pointsEdgeLeft.size() - 5; ++i) {
            kBefore = 0, kAfter = 0;
            float temp;
            //统计前5个点的斜率
            int cnt = 5;
            for (int j = i - 5; j < i; ++j) {
                temp = computeK(pointsEdgeLeft, j, j + 1);
                if (abs(temp) < 30)kBefore += temp;//忽略离散点
                else cnt--;
            }
            if (cnt == 0)continue;
            kBefore /= cnt;
            ...
            for (int j = i; j < i + 5; ++j) {
                temp = computeK(pointsEdgeLeft, j, j + 1);
                if (abs(temp) < 30)kAfter += temp;
                else cnt--;
            }
            if (cnt == 0)continue;
            kAfter /= cnt;
            //计算夹角余弦,一个向量是(1,kBefore)另外一个是（-1，-KAfter）
            float answer = (-1 - kBefore * kAfter) / ((sqrt(1 + kAfter * kAfter)) * (sqrt(1 + kBefore * kBefore)));
        ...
        if (validRowsLeft == -1)
            validRowsLeft = pointsEdgeLeft.size();

        validRowsRight = -1;
        for (int i = 5; i < pointsEdgeRight.size() - 5; ++i) {
            kBefore = 0, kAfter = 0;
            float temp;
            //统计前5个点的斜率
            int cnt = 5;
            for (int j = i - 5; j < i; ++j) {
                temp = computeK(pointsEdgeRight, j, j + 1);
                if (abs(temp) < 30)kBefore += temp;//忽略离散点
                else cnt--;
            }
            if (cnt == 0)continue;
        ...
        if (validRowsRight == -1)validRowsRight = pointsEdgeRight.size();

    }

    float computeK(vector<POINT>& pointsEdge, int i, int j) {
        if (i > pointsEdge.size() - 1 || i<0 || j>pointsEdge.size() - 1 || j < 0)return 0.0;
        float k = 0;
        float temp = pointsEdge[i].x - pointsEdge[j].x;
        if (temp == 0) {
            return 1000;
        }
        else
            return (float)((pointsEdge[i].y - pointsEdge[j].y) / temp);
    }

    float computeCCRevise(vector<POINT>& pointsEdgeLeft, vector<POINT>& pointsEdgeRight, float AreaRevise) {
        //计算左边的面积
        int AreaLeft = 0;
        for (int i = 0; i < pointsEdgeLeft.size(); ++i) {
            AreaLeft += pointsEdgeLeft[i].y;
        }
        int AreaRight = 0;
        for (int i = 0; i < pointsEdgeRight.size(); ++i) {
            AreaRight += (COLSIMAGE - pointsEdgeRight[i].y);
        }
        float area = AreaLeft - AreaRight;

        return area / pointsEdgeLeft.size() * AreaRevise;
