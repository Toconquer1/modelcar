...
public:
  ...
  void garageExitRecognition(TrackRecognition &track)
  {
    ...
    if (garageStep == GarageStep::GarageExiting) //出库中
    {
      ...
      /* pointsEdgeRight[rowBreakRight]是粉色点startPoint */
      if (track.pointsEdgeRight[rowBreakRight].x < ROWSIMAGE * 0.8) /* 避免出库提前转向优化，比例系数调整 */
      {
...
private:
  ...
  uint16_t searchBreakRight(vector<POINT> pointsEdgeRight)
  {
    ...
    if (pointsEdgeRight[0].y >= COLSIMAGE - 20) // 第一个点必须为右下点
    {
      ...
      {
        if (pointsEdgeRight[i].y <= pointsEdgeRight[rowBreakRight].y && abs(pointsEdgeRight[i].y - pointsEdgeRight[rowBreakRight].y) < 30)  /* abs差距改大 */
        {
          rowBreakRight = i;
          counter = 0;
        }
