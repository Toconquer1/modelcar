...
public:
    ...
    bool ringRecognition(TrackRecognition &track)
    {
        ...
        {
            if (track.pointsEdgeLeft.size() < 4 || track.pointsEdgeRight.size() < 4)return false;
            
            if(abs(track.stdevLeft-track.stdevRight)>40){//左右差别很大才进行判断
                ;
            }
            else {
                return false;
           ...
           */
            ...
            //如果圆环在右，左测没有突变
            //如果圆环在左，右测没有突变
            if(track.stdevLeft<40&&track.stdevRight<40){
    ...
    void drawImage(TrackRecognition track, Mat &Image)
    {
        ...
        putText(Image, "Ring", Point(COLSIMAGE / 2 - 5, 20), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0, 255, 0), 1, CV_AA); //显示赛道识别类型
    }
